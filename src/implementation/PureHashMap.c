...
void print_hash_table(){
    for (int i = 0 ; i < TABLE_SIZE; i++){
        ...
        if (hash_table[i] != NULL) {
            printf("\t\t\t%d %d %s\n", hash_table[i]->key, hash_table[i]->value1, hash_table[i]->name);
            map_entry *nextPtr = hash_table[i]->next;
            while (nextPtr != NULL) {
                printf("\t\t\t%d %d %s\n", nextPtr->key, nextPtr->value1, nextPtr->name);
...
void insert_to_hash_table(map_entry *mapEntry) {
    if (mapEntry == NULL) {
        return;
    }
...
void generate_random_string(char *str, unsigned int length) {
    unsigned int charset_size = sizeof(charset) - 1;  // exclude null terminator
